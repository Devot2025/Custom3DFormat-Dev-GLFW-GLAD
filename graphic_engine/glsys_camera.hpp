...
template <GLuint model_id, GLuint projection_id>

class Basic_Camera2D_Manager{
    
    public:
    
    //static void up_load_model(Program_Shader_Operator & pso, float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ);
    static void up_load_projection(Program_Shader_Operator & pso, float cam_dis, float aspect,float cam_depth);
    static void up_load_projection(Program_Shader_Operator & pso, SGG_Mat4 & projection_mat);
    static void up_load_model(Program_Shader_Operator & pso);
};
/*
template <GLuint model_id, GLuint projection_id>
void Basic_Camera2D_Manager<model_id, projection_id>::up_load_model(Program_Shader_Operator & pso, float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ){
    SGG_Mat4 lookAt_mat;
    sgg_lookAt(&lookAt_mat, eyeX, eyeY, eyeZ, centerX, centerY, centerZ, upX, upY, upZ);
    pso.upload_shader_uniform_mat4_data(model_id, lookAt_mat.m);
}
*/

template <GLuint model_id, GLuint projection_id>
void Basic_Camera2D_Manager<model_id, projection_id>::up_load_projection(Program_Shader_Operator & pso, SGG_Mat4 & projection_mat){
    pso.upload_shader_uniform_data(projection_id, projection_mat.m);
}
template <GLuint model_id, GLuint projection_id>
void Basic_Camera2D_Manager<model_id, projection_id>::up_load_projection(Program_Shader_Operator & pso, float cam_dis, float aspect,float cam_depth){
    SGG_Mat4 projection_mat;
    sgg_ortho(projection_mat, -cam_dis* aspect, cam_dis * aspect, -cam_dis, cam_dis, cam_depth, -cam_depth);
    pso.upload_shader_uniform_data(projection_id, projection_mat.m);
}

template <GLuint model_id, GLuint projection_id>
void Basic_Camera2D_Manager<model_id, projection_id>::up_load_model(Program_Shader_Operator & pso){
    SGG_Mat4 model;
    sgg_identity(model);
    pso.upload_shader_uniform_data(model_id, model.m);
}
...
class Basic_Camera3D_Manager{
    ...
    static void up_load_look_at(Program_Shader_Operator & pso, SGG_Mat4 & lookAt);
    
    static void up_load_perspective(Program_Shader_Operator & pso, float vertical_rad, float aspect,float near, float far);
    static void up_load_perspective(Program_Shader_Operator & pso, SGG_Mat4 & perspective);
    static void up_load_model(Program_Shader_Operator & pso);
};
...
void Basic_Camera3D_Manager<view_pos_id, perspective_id>::up_load_look_at(Program_Shader_Operator & pso, float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ){
    SGG_Mat4 lookAt_mat;
    sgg_lookAt(lookAt_mat, eyeX, eyeY, eyeZ, centerX, centerY, centerZ, upX, upY, upZ);
    pso.upload_shader_uniform_data(view_pos_id, lookAt_mat.m);
}

template <GLuint view_pos_id, GLuint perspective_id>
void Basic_Camera3D_Manager<view_pos_id, perspective_id>::up_load_look_at(Program_Shader_Operator & pso, SGG_Mat4 & lookAt){
    pso.upload_shader_uniform_data(view_pos_id, lookAt.m);
}
template <GLuint view_pos_id, GLuint perspective_id>
void Basic_Camera3D_Manager<view_pos_id, perspective_id>::up_load_perspective(Program_Shader_Operator & pso, float vertical_rad, float aspect,float near, float far){
    ...
    pso.upload_shader_uniform_data(perspective_id, perspective_mat.m);
}

template <GLuint view_pos_id, GLuint perspective_id>
void Basic_Camera3D_Manager<view_pos_id, perspective_id>::up_load_perspective(Program_Shader_Operator & pso, SGG_Mat4 & perspective){
    pso.upload_shader_uniform_data(perspective_id, perspective.m);
}
