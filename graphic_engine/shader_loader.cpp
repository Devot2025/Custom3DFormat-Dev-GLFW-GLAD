...
GLuint Basic_Shader_Loader::gen_shader_file_to_id(const char * shader_code_path, GLenum shader_type){
    ...
    if(!f_loader_manager){
        throw std::runtime_error(GET_TEMPLATE_ERROR_STRING("failed to open ") + std::string(shader_code_path) + " file.");
        return 0x0;
    }
    f_loader_manager.seekg(0, std::ios::end);
    size_t f_all_size = f_loader_manager.tellg();
    if(f_all_size <= 0x0){
        throw std::runtime_error(GET_TEMPLATE_ERROR_STRING("error : failed to get ") + std::string(shader_code_path) + " file size.");
        return 0x0;
    }
...
GLenum Basic_Shader_Loader::change_shader_type(GLenum shader_type){
    switch(shader_type){
        ...
        throw std::runtime_error(GET_TEMPLATE_ERROR_STRING("a type not int the list was used"));
    }
    return 999;
}
...
void Basic_Shader_Loader::confirm_shader_build(GLuint shader_id, GLenum basic_shader_type){
    ...
    if(basic_shader_type == BASIC_SHADER_TYPE_PROGRAM){
        ...
        if(!build_success){
            ...
            throw std::runtime_error(std::string(shader_type_str[basic_shader_type]) + " shader compile is " + err_log_buffer);
        }
    }
    else{
