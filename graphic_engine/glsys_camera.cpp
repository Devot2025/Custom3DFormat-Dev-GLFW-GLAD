...
Basic_Camera3D_Control::Basic_Camera3D_Control():first_pos_check{true}, yaw{-90.0f}, pitch{0.0f}, speed{1.0f}, sens{0.1f}, last_x{0.0f}, last_y{0.0f} {
    assignment_vector(c_pos, 0.0f, 0.0f, 20.0f);
    assignment_vector(c_front, 0.0f, 0.0f, -1.0f);
    assignment_vector(c_up, 0.0f, 1.0f, 0.0f);
}
SGG_Vector3 Basic_Camera3D_Control::get_camera_center(){
    return c_pos + c_front;
}
...
void Basic_Camera3D_Control::move_camera(Basic_GLFW_Manager & glfw_manager, float correction_value){
    constexpr float dash_scalar = 3.0f;
